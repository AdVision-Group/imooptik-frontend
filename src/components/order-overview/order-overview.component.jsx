...
const OrderOverview = ({ order, refetch, ...otherProps }) => {
	...
	const [showFinishDepositedOrderModal, setShowFinishDepositedOrderModal] =
		useState(false)
	...
	return (
		<OrderOverviewRow {...otherProps}>
			{showFinishDepositedOrderModal && (
				<FinishDepositedOrderModal
					close={() => setShowFinishDepositedOrderModal(false)}
					refetch={refetch}
					id={order._id}
					order={order}
				/>
			...
			<DeligateCol>
				...
				{showOrderDeligateModal && (
					<OrderDeligateModal
						refetch={refetch}
						close={() => setShowOrderDeligateModal(false)}
						premise={order.premises}
						id={order._id}
					/>
				)}
			</DeligateCol>
			<TableCol>
				<Link to={`/dashboard/zakaznici/${order?.orderedBy?._id}`}>
					{order?.orderedBy?.name || "Nenašiel sa"}
				</Link>
			</TableCol>
			<TableCol>{order?.orderedBy?.phone || ""}</TableCol>
			<TableCol>{translateDeliveryType(order?.deliveryType)}</TableCol>
					...
					<DropdownMenu ref={dropdownRef}>
						<ul>
							{order.status === "half-paid" && (
								<li onClick={() => setShowFinishDepositedOrderModal(true)}>
									<div>
										<GiMoneyStack />
									</div>
									Zaplatiť
