...
const updatePrice = (price, amount, isFlat) => {
	...
	if (isFlat) {
		...
		return price - formatedAmount > 0
			? parseFloat((price - formatedAmount).toFixed(0))
			: 0
	}
...
export const getTotalDiscountedPrice = (
	...
	if (!discountObj) return productDiscPrice || lensDiscPrice
	productDiscPrice = updatePrice(
		productDiscPrice,
		discountObj?.product?.flat,
		true
	)
	productDiscPrice = updatePrice(
		productDiscPrice,
		discountObj?.product?.percent,
		false
	)

	lensDiscPrice = updatePrice(lensDiscPrice, discountObj?.lenses?.flat, true)
	lensDiscPrice = updatePrice(
		lensDiscPrice,
		discountObj?.lenses?.percent,
		false
	)
...
export const isAvailable = (available) => {
	...
	if (typeof available === "number") {
		...
		return available.reduce((acc, currValue) => acc + currValue) > 0
			? true
			: false
	}
}
