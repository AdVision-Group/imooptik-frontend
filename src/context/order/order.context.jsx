...
const OrderProvider = ({ children }) => {
	...
	const saveOrder = (order, cart) => {
		const draftedOrders = localStorage.getItem("draftedOrders")
		if (!draftedOrders) {
			localStorage.setItem(
				"draftedOrders",
				JSON.stringify({ orders: [{ order, cart }] })
			)
		}
	...
	const getSavedOrder = () => {
		const draftedOrders = localStorage.getItem("draftedOrders")
		console.log("GET SAVED ORDER")
		console.log(draftedOrders)
	}
	...
	const createCombinedProducts = async () => {
					...
					...(item.discount && {
						discount: {
							...(item.discount.product && {
								product: item.discount.product["flat"]
									? { flat: formatPrice(item.discount.product.flat) }
									: { ...item.discount.product },
							}),
							...(item.discount.lenses && {
								lenses: item.discount.lenses["flat"]
									? { flat: formatPrice(item.discount.lenses.flat) }
									: { ...item.discount.lenses },
							}),
						},
					}),
		...
		try {
			...
			const response = await fetch(
				`${process.env.REACT_APP_BACKEND_ENDPOINT}/api/store/combinedProducts/createMany`,
				requestOptions
			)
